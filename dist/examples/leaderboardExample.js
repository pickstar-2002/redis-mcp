...
exports.runLeaderboardExample = runLeaderboardExample;
const redisService_1 = require("../services/redisService");
// 排行榜类
class Leaderboard {
    constructor(redisService, leaderboardName) {
    ...
    async incrementScore(userId, increment) {
        // 获取当前分数
        const currentScoreResult = await this.redisService.zrange(this.leaderboardKey, 0, -1, true);
        if (!currentScoreResult.success) {
            throw new Error('Failed to get current score');
        }
        const scores = currentScoreResult.data;
    ...
    async getRank(userId) {
        const result = await this.redisService.zrange(this.leaderboardKey, 0, -1, true);
