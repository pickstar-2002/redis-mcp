...
class MessageQueue {
  ...
  constructor(redisService: RedisService, queueName: string) {
  ...
  // 批量发送消息
  async sendMessages(messages: string[]): Promise<boolean> {
  ...
  // 批量接收消息
  async receiveMessages(count: number): Promise<string[]> {
  ...
  // 查看队列中的消息（不消费）
  async peekMessages(start: number = 0, end: number = -1): Promise<string[]> {
